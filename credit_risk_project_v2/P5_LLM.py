...
Original file is located at
    https://colab.research.google.com/drive/1tCJoBbA4g5PUA9LhAOYxo2abWZu9PT_n
"""
...
try:
    ...
    print("rbi_guidelines.txt not found — upload it via the folder icon on the left")
...
def compliance_agent(decision, profile):
    ...
    query = (f"fair lending discrimination interest rate creditworthiness "
             f"{profile.get('sector','')}" f" {profile.get('rural_urban','')}" \
             f" {profile.get('gender','')}" f" rate {decision.get('interest_rate_pct')}")
